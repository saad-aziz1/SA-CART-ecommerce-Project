...
const SignUp = () => {
  // FIXED: useDispatch() call missing parentheses
  ...
  const googleSignUp = async () => {
    try {
      ...
      // FIXED: Ab ye sahi dispatch karega
    ...
    <div className="min-h-screen bg-[linear-gradient(135deg,#0F172A_0%,#1E293B_50%,#0F172A_100%)] flex items-center justify-center p-4 py-10">
      <div className="bg-[#F8FAFC] w-full max-w-md rounded-[1.5rem] shadow-2xl border border-[#94A3B8]/20 overflow-hidden">
        <div className="bg-[#0F172A] p-8 text-center border-b border-[#F59E0B]/20">
          <h2 className="text-[#F8FAFC] text-3xl font-black tracking-tighter uppercase">SA<span className="text-[#F59E0B]">-</span>CART</h2>
        </div>
        <div className="p-6 md:p-8 space-y-4">
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[#020617] text-[10px] font-black uppercase tracking-widest">First Name</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[#020617] text-[10px] font-black uppercase tracking-widest">Last Name</label>
                <div className="relative">
            ...
            <div className="space-y-1.5">
              <label className="text-[#020617] text-[10px] font-black uppercase tracking-widest">Email</label>
              <div className="relative">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <button type="button" className="absolute right-4 top-3 text-[#94A3B8]" onClick={() => setShowPassword(!showPassword)}>
                  {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <button disabled={isLoading} type="submit" className="w-full bg-[#F59E0B] text-[#020617] py-3.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-[#0F172A] hover:text-[#F8FAFC] transition-all flex items-center justify-center gap-2 shadow-lg mt-4 disabled:opacity-70">
          ...
          <button type="button" className="w-full bg-white border border-[#94A3B8]/20 py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-3 hover:border-[#F59E0B] transition-all" onClick={googleSignUp}>
            <img src="https://www.svgrepo.com/show/475656/google-color.svg" className="w-4 h-4" alt="Google" /> Continue with Google
          </button>
          <p className="text-center mt-6 text-xs text-[#94A3B8]">Already a member? <Link to="/login" className="text-[#0F172A] font-black hover:text-[#F59E0B] ml-1 transition-colors underline">Login Here</Link></p>
        </div>
      </div>
    </div>
  );
